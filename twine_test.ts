...
Deno.test({
  ...
  fn: async () => {
    ...
    const s_listener = await Deno.listen({
      path: sock_path,
      transport: "unix",
    });
    ...
    (async function () {
      ...
      const rcv_payload_str = new TextDecoder().decode(
        buf.slice(5, 5 + p_size),
      );
    ...
    await t._send(
      ...
      service,
      command,
      new TextEncoder().encode(payload_str),
    );
...
Deno.test({
  ...
  fn: async () => {
    ...
    // @ts-ignore setting private t.conn externally
    twine_client.conn = await Deno.connect({
      path: sock_path,
      transport: "unix",
    });
...
Deno.test({
  ...
  fn: async () => {
    ...
    const replyP1 = twine_server.sendBlock(
      7,
      11,
      new TextEncoder().encode("test payload numero uno"),
    );
    const replyP2 = twine_server.sendBlock(
      7,
      12,
      new TextEncoder().encode("test payload numero dos"),
    );
